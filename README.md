...
`npp-secops-sim` 是“核电厂管理网络告警研判智能体与工作流协同仿真平台”的 V0.2 最小可运行版本。项目面向硕士论文《面向核电厂管理网络的告警研判智能体与工作流协同安全运维方法研究》的第四章仿真验证，重点验证离线告警研判、风险评分、受限自动处置仿真、工作流触发和实验指标统计闭环。
...
- 使用 `WorkflowEngine` 根据风险等级触发归档、工单、受限自动处置仿真、人工复核或升级处置流程。
...
本项目只进行离线仿真验证，不连接真实核电厂管理网络，不访问外部互联网，不执行任何真实攻击行为。项目不会生成真实攻击代码或真实 WebShell，不执行端口扫描、漏洞利用、爆破、提权、横向移动等操作。WebShell 场景只使用文件路径、上传目录、访问行为、关联告警和风险标签等元数据进行模拟。`high` 与 `critical` 风险事件只能触发人工复核或升级处置流程，不能自动执行封禁、删除、隔离等真实动作。V0.2 中的 `simulated_block_ip` 仅表示“仿真临时封禁 IP”的审计记录，不连接真实网络、不下发真实安全策略。
...
需要注意的是，V0.2 的结果依赖模拟数据和规则权重，适合验证方法流程、可解释性、受限自动处置仿真和闭环机制，不应表述为真实核电厂管理网络中的最终检测能力。
...
该界面仅用于离线仿真展示，不代表真实核电厂管理网络运行界面，不连接真实网络系统，不执行真实攻击行为，不生成真实恶意代码。界面中的“仿真临时封禁IP”只表示受限自动处置的离线审计记录，不会执行真实封禁、删除或隔离。
...
1. **平台概览**：展示告警总数、有效告警、高风险告警、人工复核、受限自动处置仿真、闭环流程、平均研判时间，以及事件类型、风险等级和工作流动作分布。
